/**
 * IPlugInstrument - iPlug2 Wasm AudioWorklet Processor
 *
...
class IPlugInstrumentProcessor extends AudioWorkletProcessor {
  constructor(options) {
    super();
    ...
    if (!this.Module) {
      console.error('IPlugInstrument: DSP Module not found in globalThis');
      return;
    }
    ...
    if (!this.instanceId) {
      console.error('IPlugInstrument: Failed to create DSP instance');
      return;
    }

    console.log('IPlugInstrument: Created instance', this.instanceId);
  ...
  _cleanup() {
    ...
    if (this.instanceId) {
      console.log('IPlugInstrument: Destroying instance', this.instanceId);
  ...
  _sendPluginInfo() {
    ...
    if (this.Module.getPluginInfoJSON) {
      try {
        ...
        console.log('IPlugInstrument Processor: pluginInfo JSON for instance', this.instanceId);
        const pluginInfo = JSON.parse(jsonStr);
        this.port.postMessage({ verb: 'pluginInfo', data: pluginInfo });
      } catch (e) {
        console.warn('IPlugInstrument: Failed to send plugin info:', e);
      }
    }
  }
  ...
  process(inputs, outputs, parameters) {
    ...
    // Zero input/output buffers up front so any channel the upstream didn't
    // supply this quantum (or any output the plugin doesn't fully write) is
    // silent instead of replaying stale samples from a previous block.
    ...
    for (let ch = 0; ch < this.numOutputChannels; ch++) {
      Module.HEAPF32.fill(0, this.outputPtrs[ch] >> 2, (this.outputPtrs[ch] >> 2) + bufsize);
    }

    // Copy whatever input channels the upstream did supply over the top.
...
registerProcessor('ipluginstrument-processor', IPlugInstrumentProcessor);
