/**
 * IPlugInstrument - iPlug2 Wasm Web Component Bundle
 *
...
window.Module = window.Module || {
  preRun: [],
  postRun: [],
  print: text => console.log('[IPlugInstrument]', text),
  printErr: text => console.error('[IPlugInstrument] stderr:', text)
};

(function() {
  'use strict';

  const PLUGIN_NAME = 'IPlugInstrument';
  const PLUGIN_NAME_LC = 'ipluginstrument';
  const PLUGIN_TAG = 'iplug-ipluginstrument';
  const NUM_INPUTS = 0;
  const NUM_OUTPUTS = 2;
  const IS_INSTRUMENT = true;
  const HOST_RESIZE = false;
  const HAS_UI = true; // false for headless plugins
  const DOES_MIDI_IN = true;
  const DOES_MIDI_OUT = true;
  ...
  class IPlugWasmController {
    ...
    async init(options = {}) {
      ...
      try {
        ...
        } else if (options.useSAB !== false) {
          console.warn('IPlugWasmController: SharedArrayBuffer unavailable. Serve with COOP/COEP headers for low-latency metering.');
        }
        ...
        case 'SMMFD': // Send MIDI Message From Delegate
          {
            ...
            if (typeof msg.data === 'string') {
              // Legacy string format: "status:data1:data2"
              const parts = msg.data.split(':').map(Number);
              [status, data1, data2] = parts;
            } else {
              // New object format with separate fields
              status = msg.status;
              data1 = msg.data1;
              data2 = msg.data2;
            }
            ...
            if (this.uiModule?.SMMFD) this.uiModule.SMMFD(status, data1, data2);
          }
          break;
     ...
     */
    _routeSABMessage(msg) {
      switch (msg.msgType) {
        case 0: { // SCVFD - Send Control Value From Delegate
          const value = msg.payload && msg.payload.length >= 4
            ? new DataView(msg.payload.buffer, msg.payload.byteOffset, 4).getFloat32(0, true)
            : 0;
          this.emit('controlValue', { ctrlTag: msg.ctrlTag, value });
          if (this.uiModule?.SCVFD) this.uiModule.SCVFD(msg.ctrlTag, value);
          break;
        }

        case 1: // SCMFD - Send Control Message From Delegate
          this.emit('controlMsg', { ctrlTag: msg.ctrlTag, msgTag: msg.msgTag, data: msg.payload });
          if (this.uiModule?.SCMFD && this.uiModule?.HEAPU8 && msg.payload) {
            const ptr = this.uiModule._malloc(msg.payload.length);
            this.uiModule.HEAPU8.set(msg.payload, ptr);
            this.uiModule.SCMFD(msg.ctrlTag, msg.msgTag, msg.payload.length, ptr);
            this.uiModule._free(ptr);
          }
          break;

        case 2: // SAMFD - Send Arbitrary Message From Delegate
          this.emit('arbitraryMsg', { msgTag: msg.msgTag, data: msg.payload });
          if (this.uiModule?.SAMFD && this.uiModule?.HEAPU8 && msg.payload) {
            const ptr = this.uiModule._malloc(msg.payload.length);
            this.uiModule.HEAPU8.set(msg.payload, ptr);
            this.uiModule.SAMFD(msg.msgTag, msg.payload.length, ptr);
            this.uiModule._free(ptr);
          }
          break;
      }
    ...
    _pollSAB() {
      ...
      try {
        ...
        while (readIdx < writeIdx) {
          ...
          let payload = null;
          if (dataSize > 0) {
            payload = new Uint8Array(new Uint8Array(this.sabBuffer, offset + msgHeaderSize, dataSize));
          }

          const msg = { msgType, ctrlTag, msgTag, payload };
          const shouldCoalesce = msgType === 0 || (msgType === 1 && msgTag === 0);

          if (shouldCoalesce) {
            const key = `${msgType}:${ctrlTag}:${msgTag}`;
            if (!coalescedMessages.has(key)) {
              pendingMessages.push({ key });
            }
            coalescedMessages.set(key, msg);
          } else {
        ...
        for (const pending of pendingMessages) {
          this._routeSABMessage(pending.key ? coalescedMessages.get(pending.key) : pending.msg);
        }
      } catch (e) {
