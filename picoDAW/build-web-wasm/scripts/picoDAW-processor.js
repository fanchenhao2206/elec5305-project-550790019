/**
 * picoDAW - iPlug2 Wasm AudioWorklet Processor
 *
...
class picoDAWProcessor extends AudioWorkletProcessor {
  constructor(options) {
    super();
    ...
    if (!this.Module) {
      console.error('picoDAW: DSP Module not found in globalThis');
      return;
    }
    ...
    if (!this.instanceId) {
      console.error('picoDAW: Failed to create DSP instance');
      return;
    }

    console.log('picoDAW: Created instance', this.instanceId);
  ...
  _cleanup() {
    ...
    if (this.instanceId) {
      console.log('picoDAW: Destroying instance', this.instanceId);
  ...
  _sendPluginInfo() {
    ...
    if (this.Module.getPluginInfoJSON) {
      try {
        ...
        console.log('picoDAW Processor: pluginInfo JSON for instance', this.instanceId);
        const pluginInfo = JSON.parse(jsonStr);
        this.port.postMessage({ verb: 'pluginInfo', data: pluginInfo });
      } catch (e) {
        console.warn('picoDAW: Failed to send plugin info:', e);
      }
    }
  }
...
registerProcessor('picodaw-processor', picoDAWProcessor);
